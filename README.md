...
After `jsp login`, the app is four small commands, and each one is a useful
exit ramp on its own:

* `jsp fetch` prints a portable JSON snapshot of your stats;
* `jsp render` turns that snapshot into a native-size PNG — no hardware needed;
* `jsp display` pushes a rendered image to a locally attached panel;
* `jsp serve` exposes the JSON and PNG over HTTP for TRMNL or any other
  remote consumer.
...
```console
git clone https://github.com/Automattic/jetpack-stats-on-paper.git
cd jetpack-stats-on-paper
uv sync --locked
cp .env.example .env
```
...
```console
uv run jsp login --manual
```

`jsp` prints an authorization URL. Open it, approve access for your site, and
paste the full URL you are redirected to back into the terminal. The token is
stored privately in `~/.config/jsp/token.json`. By default the app requests
...
```console
JSP_TOKEN_PATH=~/.config/jsp/token-example.com.json \
  WPCOM_SITE=example.com uv run jsp fetch
```
...
```console
uv run jsp fetch
uv run jsp render --panel impression-7in3 -o out.png
uv run jsp serve
```

`jsp serve` exposes `GET /v1/stats.json` and `GET /v1/screen.png?panel=<profile>`
on localhost. Set `JSP_SERVE_TOKEN` to require a bearer token — without one the
...
Snapshots are cached for an hour by default (`JSP_POLL_INTERVAL_SECONDS`). The
...
- `JSP_SOURCE=direct|url` — read WordPress.com directly, or poll another
  `jsp serve` instance at `JSP_SOURCE_URL`.
- `JSP_TZ` — the site's timezone, so "today" rolls over when the site's day
  does (default `Europe/Madrid`).
- `JSP_SERIES_DAYS` — how many daily points to fetch (default 30). A panel
  draws as many of the most recent days as it can show legibly and captions
  exactly that window.
- `JSP_CACHE_DIR` — where the last snapshot is kept (default `~/.cache/jsp`).
- `JSP_VIEW` — which screen to draw, `stats` or `commerce` (default `stats`).
- `JSP_PANEL` — which panel `render` and `display` target, so a device is
  configured once rather than passed on every command.
- `JSP_COMMERCE` — fetch the WooCommerce order series. `JSP_VIEW=commerce`
  needs it set on whichever process does the fetching.

Both `JSP_VIEW` and `JSP_PANEL` are plain environment variables, which is what
...
chart. It needs `JSP_COMMERCE=true` and a site with a store.

![Commerce view rendered for an 800×480 Spectra 6 panel](assets/previews/commerce-impression-7in3.png)

```console
uv run jsp render --view commerce --panel impression-7in3 -o orders.png
```

### Display on a panel

```console
uv run jsp display --panel impression-7in3
```
